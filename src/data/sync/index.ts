...
const BUILDERS: Record<string, RowBuilder> = {
  ...
  exercises: (row) => ({
    sql: `INSERT OR REPLACE INTO exercises (id, phase_id, name, detail, sets, reps, duration_s, exercise_type, sort_order, video_url, how_to, warmup_sets, archived_at, equipment_type, target_rpe, rest_s, synced)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)`,
    bind: [row.id, row.phase_id, row.name, row.detail ?? null, row.sets ?? null,
           ...
           row.equipment_type ?? "none", row.target_rpe ?? null, row.rest_s ?? null],
  }),
  phase_criteria: (row) => ({
